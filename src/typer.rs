...
impl TyInferencePrePass {
    ...
    fn fill_cxt(&mut self, ast: &AST) {
        for d in ast.into_iter() {
            if let Decl::Fn(f) = d {
                self.cxt.insert_global(
                    f.header.name.lexeme.clone(),
                    Ty {
                        kind: f.into(),
                        span: f.span.clone(),
                    },
    ...
    fn get_struct_ty(&mut self, key: &str) -> Option<Struct> {
        match self.ty_table.get(key) {
            Some(t) => match t {
                TyDecl::Struct(s) => Some(s.clone()),
                _ => None,
            },
            _ => None,
        }
    ...
    fn match_assign(&mut self, target: &AssingKind, span: &Span) -> Ty {
        match target {
            AssingKind::Var(var) => match self.cxt.get(&var.lexeme) {
                Some(t) => t.clone(),
                None => {
                    self.span_err(
                        ErrKind::Type(TypeErr::VarNotFound(var.lexeme.clone())),
                        var.span.clone(),
                    );
                    Ty::default_unit_type(span.clone())
                }
            },
            AssingKind::Field {
                ref callee,
                ...
                let ty = self.match_assign(callee, span);
                if let TyKind::Path(p) = ty.kind {
                    let first = p.first().unwrap();
                    match self.get_struct_ty(&first.lexeme).unwrap().fields.get(first) {
                        Some(s) => s.clone(),
                        None => Ty::default_unit_type(span.clone()),
                    }
                } else {
                    self.span_err(
                        ErrKind::Type(TypeErr::FieldNotFound {
                            ty: ty.kind,
            ...
            AssingKind::Index { ref callee, .. } => {
                let ty = self.match_assign(callee, span);
                if let TyKind::Array(elem) = ty.kind {
                    *elem.clone()
                } else {
                    self.span_err(
                        ErrKind::Type(TypeErr::InvalidType {
                            expected: TyKind::Array(box Ty::default_unit_type(span.clone())),
                            actual: ty.clone(),
                        }),
                        span.clone(),
                    );
    ...
    fn infer_struct_lit(&mut self, name: &Ident, members: &mut Vec<Member>) {
        let ty = self.get_struct_ty(&name.lexeme);
        match ty {
            Some(s) => {
                for member in members {
                    match s.fields.get(&member.name) {
                        Some(t) if t.kind == member.init.ty.kind => {}
                        Some(t) => {
                            self.span_err(
                                ErrKind::Type(TypeErr::InvalidType {
                                    expected: t.kind.clone(),
            ...
            None => {
                ...
                self.span_err(
                    ErrKind::Type(TypeErr::TyNotFound(name.lexeme.clone())),
                    name.span.clone(),
                );
            }
        }
    ...
    fn visit_decl(&mut self, decl: &mut Decl) -> Self::Result {
        match decl {
            ...
            Decl::TyDecl(TyDecl::Struct(s)) => {
                s.methods.iter_mut().for_each(|(_, f)| self.infer_fn(f))
            }
            Decl::TyDecl(TyDecl::Enum(e)) => {
                e.methods.iter_mut().for_each(|(_, f)| self.infer_fn(f))
            }
            _ => {}
        }
    }
    fn visit_stmt(&mut self, stmt: &mut Stmt) -> Self::Result {
        match stmt {
            ...
            Stmt::Assign {
                ...
                self.infer_expr(rhs);
                if self.match_assign(&target.kind, &target.span).kind != rhs.ty.kind {
                    self.span_err(
                        ErrKind::Type(TypeErr::InvalidType {
        ...
        match expr.node {
            ...
            ExprKind::Logical {
                ...
                match op {
                    ...
                    CmpOp::EqEq | CmpOp::NotEq if lhs.ty.kind != rhs.ty.kind => {
                        self.span_err(
                            ErrKind::Type(TypeErr::InvalidType {
                                actual: lhs.ty.clone(),
                                expected: rhs.ty.kind.clone(),
            ...
            ExprKind::Index {
                ...
                if let TyKind::Array(ref mut elem) = callee.ty.kind {
                    expr.ty.kind = elem.kind.clone();
                    return;
                } else {
                    self.span_err(
                        ErrKind::Type(TypeErr::InvalidType {
            ...
            ExprKind::Struct {
                ...
                expr.ty.kind = TyKind::Path(Path::new(vec![name.clone()], expr.span.clone()))
            }
            ExprKind::Var(ref var) | ExprKind::This(ref var) => match self.cxt.get(&var.lexeme) {
                Some(t) => {
                    expr.ty.kind = t.clone().kind;
                }
                None => {
                    self.span_err(
                        ErrKind::Type(TypeErr::VarNotFound(var.lexeme.clone())),
            ...
            ExprKind::Call {
                ...
                self.infer_expr(callee);
                if let ExprKind::Field(ref call_arg, field) = &callee.node {
                    args.insert(0, *call_arg.clone());
                }
                if let TyKind::Fn(params, ret) = &callee.ty.kind {
                    ...
                    for (p, a) in params.iter().zip(args.iter_mut()) {
                        if p.kind != a.ty.kind {
                            self.span_err(
                                ErrKind::Type(TypeErr::InvalidType {
                                    actual: a.ty.clone(),
                                    expected: p.kind.clone(),
                                }),
                                a.span.clone(),
                            );
                        }
                        a.ty.kind = p.kind.clone();
                    }
                    expr.ty.kind = ret.kind.clone();
                } else {
                    let arg_types = args.iter().map(|a| a.ty.clone()).collect();
                    self.span_err(
                        ErrKind::Type(TypeErr::InvalidType {
                            expected: TyKind::Fn(
                                arg_types,
            ...
            ExprKind::Path(ref path) => {
                ...
                match self.ty_table.get(&ty_name.lexeme) {
                    ...
                    None => {
                        self.span_err(
                            ErrKind::Type(TypeErr::TyNotFound(ty_name.lexeme.clone())),
                            ty_name.span.clone(),
                        );
                    }
                }
            ...
            ExprKind::Field(ref mut callee, ref name) => {
                self.infer_expr(callee);
                if let TyKind::Path(ref p) = callee.ty.kind {
                    let first = p.first().unwrap();
                    if let Some(s) = self.get_struct_ty(&first.lexeme) {
                        match s.fields.get(name) {
                            Some(t) => {
                                expr.ty.kind = t.kind.clone();
                                return;
                            }
                            None => {
                                if let Some(f) = s.methods.get(&name.lexeme) {
                                    expr.ty.kind = f.into();
                                    return;
                                } else {
                                    self.span_err(
                                        ErrKind::Type(TypeErr::FieldNotFound {
                                            ty: TyKind::Path(p.clone()),
                                            field: name.lexeme.clone(),
                                        }),
                                        name.span.clone(),
                                    );
